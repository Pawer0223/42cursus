...
- 0이될 때까지의 과정에서의 `정수부`(0 or 1)를 마지막부터 차례대로 나열하면 된다.
  - ex) 12.125일 때
  - `정수부` => `1100`
  - `실수부` 
      => `0.125` x 2 => `0`.25
      => `0.25` x 2 => `0`.5
      => `0.5` x 2 => `1`.0
      => 따라서 실수부는 2진수로 `100`으로 표현.
  - 12.125 => `1100.100`

### 고정 소수점방식?
- `정수`와 `실수`를 표현하는 비트수를 정해놓고 해석한다.
- 만약 실수를 4byte공간에 저장할 때, `1bit = 부호`, `16bit = 정수`, `15bit는 실수`를 표현한다고 `가정`한다면
- 실수 `12.125`는 2진수로 `1100.100`이기 때문에 `실수`에 해당하는 2진수 `100`은 15비트 공간에 저장 된다. (뒷자리는 0으로 채움)
...
- 쉽게 위에서 `12.125`의 2진수 `1100.100`을 `1.100100` * 2^`n`으로 나타내도록 하는것이다.
  - 정확하게 얘기하면 `정수부가 1만 남을때까지(중요)` 소수점을 이동시킨다. (왼쪽 or 오른쪽) 그리고 `이동한 칸 수만큼 n 자리`에 집어 넣으면 된다.
  -	즉, 예시에서 n은 3이 된다.
  - 따라서 1.`100100`에서 `2^3` 이동한 후부터의 bit는 `실수`를 표현함을 구분할 수 있다.
  ...
  - 만약 실수를 4byte공간에 저장할 때, `1bit = 부호`, `8bit = n`, `23bit는 이진수`를 표현한다고 한다면?
    - `23bit`공간에 이진수를 표현할 수 있다.
      ...
      - 이 때 맨앞의 1은 생략하고 나머지를 23bit로 표현한다. 예시에서도 1.100100일 때 1을 제외한 `100100 ... ` 이 채워진다.
    ...
    - `8bit`공간에는 실수 자료형이 표현된 비트를 구분할 수 있는 2^`n`의 값을 저장할 수 있다. 
      ...
      - `1(hidden bit) 000 0000 0000 0000 0000 0001` `0000 00` 과 같이 6bit가 0으로 채워짐으로써, 가수부에서 나타내는 정수가 더 큰 값을 가지고 있다는 것을 의미하게 된다. ( TEST 해보자)
